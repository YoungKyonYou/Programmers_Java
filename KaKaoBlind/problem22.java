...
public class problem22 {
    ...
    public static int solution(int n, int k) {
        ...
        for (int i = 0; i < ans.length(); i++) {
            if (ans.indexOf("0", idx + 1) != -1) {
                idx = ans.indexOf("0", idx + 1);
                for (int j = idx - 1; j >= s; j--) {

                    if (ans.charAt(j) == '0')
                        break;
                    t = Character.toString(ans.charAt(j)) + t;
                    Long tempInt = Long.parseLong(t);
                    System.out.println("tempInt!:" + tempInt);
                    if (isPrime(tempInt)) {
                        System.out.println("It is Prime!!:" + tempInt);
                        answer++;
                        tempStr = "";
                        break;
                    }
                ...
                t = "";
                //  ans=ans.substring(idx+1, endIndex)
            } else {
                flag = 1;
                break;
            }
        ...
        if (flag == 1) {
            for (int j = idx; j <ans.length(); j++) {
                t += Character.toString(ans.charAt(j));
                Long tempInt = Long.parseLong(t);
                System.out.println("tempInt:" + tempInt);
                if (isPrime(tempInt)) {
                    answer++;
